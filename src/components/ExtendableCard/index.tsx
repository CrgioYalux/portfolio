			...
			<div className="flex-none basis-10 flex flex-row gap-4 justify-between items-center">
				<div className="flex flex-row gap-2 items-center">
					...
					<a
						href={deploy}
						target="_blank"
						className="cursor-pointer text-lg font-bold md:hover:underline"
					>
				...
				>
					{
						<AngleDownIcon
							className={`h-4 w-4 fill-current pt-0.5 ${isOpen ? 'rotate-180' : 'rotate-0'} transition-transform duration-100`}
						/>
					}
				</button>
			</div>
			<div
				className={`grow-0 shrink ${isOpen ? 'basis-full py-2' : 'basis-0'} border-2 border-primary px-2 text-sm text-text font-medium bg-primary rounded-md transition-[padding] duration-100 ${className}`}
			>
				{isOpen ? children : ''}
			</div>
		</div>
